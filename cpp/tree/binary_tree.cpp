...
template <typename T>
typename BinaryTree<T>::const_iterator& BinaryTree<T>::const_iterator::
operator++() {
  ...
  if (!node->parent || node->right) {
    ...
    // If we ascend to the root, and it is the current node (node == root) or
    // less than the current node (node == endNode)
...
void BinaryTree<T>::printInOrder(
    const BinaryTree<T>::TreeNode* const node) const {
  if (!node) return;
...
template <typename T>
typename BinaryTree<T>::TreeNode* const BinaryTree<T>::find(
    const T val, BinaryTree<T>::TreeNode* const node) const {
  if (!node) {
    return nullptr;
  }
  if (node->val == val) {
...
template <typename T>
typename BinaryTree<T>::TreeNode* const BinaryTree<T>::erase(
    const T val, BinaryTree<T>::TreeNode* const node) {
  if (!node) {
    return nullptr;
  }
  if (node->val == val) {
    ...
    // Else, replace both subtrees and repair prev--x-->repl->right to
    // prev->right
