...
public:
    ...
    const_iterator find(const T val) const { return const_iterator(find(val, root.get())); }

    void insert(const T val);
    void insert(std::initializer_list<T> il) { for (auto v : il) insert(v); }

    void erase(const T val);
    void erase(std::initializer_list<T> il) { for (auto v : il) erase(v); }
