...
class Trie:
    @classmethod
    def __search(cls, word, is_prefix, node, pos=0):
        if pos == len(word):
            if is_prefix: return True
            return True if node.is_word else False
        if word[pos] not in node.children: return False
        return cls.__search(word, is_prefix, node.children[word[pos]], pos + 1)

    def __init__(self):
        self.root = Node(None)

    def __contains__(self, prefix):
        return self.__search(prefix, True, self.root)
    ...
    def has_word(self, word):
        return self.__search(word, False, self.root)
